...
gridcellarea = xr.open_dataset(
    root / "raw_data/gridcellarea-em-biomassburning_input4MIPs_emissions_CMIP_VUA-CMIP-BB4CMIP6-1-2_gn.nc"
).gridcellarea
...
def calculate_annual_series(nc_filepath):
    ...
    for year in tqdm(years):
        ...
        days_in_year = [v[1] for v in
            [monthrange(int(i.dt.year.values), int(i.dt.month.values)) for i in sel.time]
        ]
        sel_kg_s = sel * gridcellarea
        sel_monthly_kg = sel_kg_s.sum(dim=["latitude", "longitude"]).to_series() * days_in_year * 24 * 3600
        sel_annual_Tg = sel_monthly_kg.sum() / 10**9
        series.at[year] = sel_annual_Tg

    print("\t Check first value: ",
          np.round(series.iloc[0], 2), float(ds.annual_total_first_year_Tg_yr))
    assert np.round(series.iloc[0], 2) == float(ds.annual_total_first_year_Tg_yr)

    print("\t Check last value: ",
        np.round(series.iloc[-1], 2), float(ds.annual_total_last_year_Tg_yr))
